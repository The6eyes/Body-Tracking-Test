...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARBodyAnchor) {
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            ...
            // Create an entity for the joint, add to joints directory, and add it to the parent entity (i.e. bodySkelton)
            let jointEntity = createJoint(radius: jointRadius, color: jointColor)
            joints[jointName] = jointEntity
            self.addChild(jointEntity)
        }
        for bone in Bones.allCases {
            guard let skeltonBone = createSkeletonBone(bone: bone, bodyAnchor: bodyAnchor)
                    else { continue }
            
            //create an entity for the bone, add to bones direectory, and add it to the parent entity (i.e. bodySkeleton)
            let boneEntity = createBoneEntity(for: skeltonBone)
